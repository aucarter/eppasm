function pop_project_one_step!(β, t, x, out_dict)
  ...
  @. x[:hivpop][:, 2:end, :] .+=  hiv_ag_prob[:, 1:end-1] * last_hivpop[:, 1:end-1, :]
  @. x[:hivpop][:, 1:end-1, :] .-=  hiv_ag_prob[:, 1:end-1] * last_hivpop[:, 1:end-1, :]
  if t > β[:tARTstart]
    @. x[:artpop][:, 2:end, :, :] .+=  hiv_ag_prob[:, 1:end-1] * last_artpop[:, 1:end-1, :, :]
    @. x[:artpop][:, 1:end-1, :, :] .-=  hiv_ag_prob[:, 1:end-1] * last_artpop[:, 1:end-1, :, :]
  end
